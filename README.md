...
On top of that, we also do version fallback. This means that if a file used in version 1.0.1 is no longer available in 1.0.2 and a user requests the nonexistent file, we will fall back to the previous 1.0.1 version and serve it instead of failing with a 404 error.
